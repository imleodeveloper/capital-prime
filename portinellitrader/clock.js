document.addEventListener("DOMContentLoaded", () => {
  ...
  const liveHours = [13, 20, 22]; // pode deixar isso em qualquer ordem
  ...
  function updateClock() {
    ...
    if (liveCheck.isLive) {
      clockSpan.innerHTML = `Estamos ao vivo! <br/> Próxima live em breve.`;
      return;
    }
    ...
    clockSpan.innerHTML = `Faltam ${format(hours)}:${format(minutes)}:${format(
      seconds
    )} para <br /> live das ${label}`;
  }
